...
class Mod_scores extends Model {
    ...
    function get_students($p=null) {
        ...
        $this->db->order_by($db->stu_first_name, 'asc');
        $this->db->group_by($db->stu_id, 'desc');
        if ($p['type'] == 'edit') {
        ...
        if ($data->num_rows() > 0) {
            foreach ($data->result() as $row) {
                ...
                $field = array(
                    ...
                    $db->sco_unfocused => $this->input->post($db->sco_unfocused . $row->stu_id),
                    $db->sco_discruptive => $this->input->post($db->sco_discruptive . $row->stu_id),
                    $db->sco_withdrawn => $this->input->post($db->sco_withdrawn . $row->stu_id),
                    $db->sco_improve => $this->input->post($db->sco_improve . $row->stu_id),
